...
const FEATURES = [
  ...
  { icon: "📊", title: "Handles Any Video Size",       desc: "Works on videos with hundreds or hundreds of thousands of comments. All entries are loaded and stored before the random selection is made." },
  ...
  { icon: "🚫", title: "Duplicate Removal",            desc: "One entry per YouTube account. Prevents the same user from appearing multiple times in the random pool regardless of how many comments they left." },
  { icon: "#️⃣", title: "Keyword Qualification",       desc: "Restrict the random pool to comments containing a required word, hashtag or phrase so only valid entries are included." },
  ...
  { icon: "⏱️", title: "Time Window Restriction",     desc: "Set an entry window — only comments posted within the first N minutes after the video published are included in the random pool." },
  ...
  { icon: "🏆", title: "Multi-Winner Draws",           desc: "Pick multiple random winners from a single draw. All winners appear on one verification page with their comment text shown." },
];
...
const STEPS = [
  ...
  { num: "03", title: "Filter Your Entry Pool",        desc: "Optionally add a keyword filter, emoji filter or time window. Enable duplicate removal to ensure one entry per account. The valid entry count updates live." },
  ...
  { num: "05", title: "Verify and Share",              desc: "Copy the public verification link generated for every draw. Share it in your video description or comment section as permanent proof." },
];

const FAQS = [
  { q: "How random is the selection?",                               a: "The tool uses a cryptographically random algorithm. Every valid entry in the pool has an identical probability of selection — the result cannot be predicted or manipulated." },
  ...
  { q: "What counts as a valid entry?",                              a: "A valid entry is any public comment that passes all active filters — keyword, emoji, time window and duplicate removal. The valid entry count is shown before you pick so you know exactly how many entries are in the pool." },
  ...
  { q: "Can I use filters and still keep the selection random?",     a: "Yes. Filters only restrict the pool of eligible entries. The random selection within that filtered pool remains fully random — every qualifying comment has an equal chance." },
  ...
  return (
    <main className="min-h-screen bg-[#0B0F19] text-white px-6 py-16">
      <div className="max-w-5xl mx-auto">
        ...
        {/* ── HERO ─────────────────────────────────────────────────── */}
        <p className={S.label}>Random Winner Generator</p>
        ...
        <section className={S.section}>
          ...
          <h2 className={`${S.h2} mt-3`}>What is a Random YouTube Comment Picker?</h2>
          <div className="mt-6 grid gap-5 text-zinc-400 leading-relaxed text-base max-w-3xl">
            <p>
              A random YouTube comment picker is a tool that loads all comments
              from a YouTube video and selects one or more winners using a
              verifiably random algorithm. It removes the guesswork, manual effort
              and bias from choosing giveaway winners.
            </p>
            <p>
              Most random pickers stop at the winner name. YT Giveaway Picker
              goes further — it generates a permanent public verification page
              for every draw, showing your audience exactly how the winner was
              chosen, what filters were applied and how many valid entries were
              in the pool. This makes disputes impossible and builds genuine
              trust with your viewers.
            </p>
            <p>
              The tool supports both standard YouTube videos and YouTube Shorts,
              handles comment pools of any size and lets you stack multiple
              filters to define exactly which comments qualify as valid entries
        ...
        <section className={S.section}>
          ...
          <h2 className={`${S.h2} mt-3`}>How to Pick a Random YouTube Comment Winner</h2>
        ...
        {/* ── COMPARISON TABLE ─────────────────────────────────────── */}
        <section className={S.section}>
          <p className={S.label}>Comparison</p>
          <h2 className={`${S.h2} mt-3`}>Why Use This Over Other Methods?</h2>
          <div className="mt-8 overflow-x-auto">
            <table className="w-full text-sm text-left border border-white/[0.07] rounded-2xl overflow-hidden">
              <thead>
                <tr className="bg-[#111827] border-b border-white/[0.07]">
                  <th className="px-5 py-4 text-white font-black">Method</th>
                  <th className="px-5 py-4 text-white font-black text-center">Truly Random</th>
                  <th className="px-5 py-4 text-white font-black text-center">Publicly Verified</th>
                  <th className="px-5 py-4 text-white font-black text-center">Instant</th>
                </tr>
              </thead>
              <tbody>
                {COMPARE.map((row, i) => (
                  <tr key={row.method} className={`border-b border-white/[0.05] ${i === COMPARE.length - 1 ? "bg-lime-400/[0.04]" : ""}`}>
                    <td className={`px-5 py-4 font-bold ${i === COMPARE.length - 1 ? "text-lime-400" : "text-zinc-300"}`}>{row.method}</td>
                    <td className="px-5 py-4 text-center">{row.random    ? "✅" : "❌"}</td>
                    <td className="px-5 py-4 text-center">{row.verified  ? "✅" : "❌"}</td>
                    <td className="px-5 py-4 text-center">{row.fast      ? "✅" : "❌"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <section className={S.section}>
          ...
          <h2 className={`${S.h2} mt-3`}>Everything in the Random Comment Picker</h2>
        ...
        <section className={S.section}>
          <div className={`${S.card} bg-lime-400/[0.03] border-lime-400/[0.12]`}>
            ...
            <h2 className={`${S.h2} mt-3 text-2xl`}>Other Pickers on YT Giveaway Picker</h2>
            <p className={`${S.body} mt-3 max-w-xl`}>
              The same random selection engine powers dedicated pickers for
              standard YouTube videos and YouTube Shorts — all with the same
              verification system and winner card downloads.
            </p>
            <div className="flex flex-wrap gap-4 mt-6">
              <a href="/youtube-comment-picker" className={S.cta}>
                YouTube Comment Picker →
              </a>
              <a href="/youtube-shorts-giveaway-picker" className={S.ctaGhost}>
        ...
        <section className={`${S.section} mb-8`}>
          <div className="rounded-2xl bg-lime-400/[0.04] border border-lime-400/[0.12] p-10 text-center">
            <h2 className="text-3xl md:text-4xl font-black">
              Generate Your Random Winner Now
            </h2>
            <p className={`${S.body} mt-4 max-w-md mx-auto`}>
              Free forever. No signup. Paste any YouTube URL and pick a
              verified random winner in seconds.
            </p>
